...
#
# History
# 02/03/2020 Fixing for lines 78, 159/160/161
#
# 19/02/2020 Removed debug option, i.e. matplotlib import and any print statements, because they are not useful for the web interface-- MI 
#
...
#
# For future updates:
# 1. Do not modify the return statement and the main() parameter list order. If any change, include the specific change in the email to bring the script online.
# 2. If any new imports is added to the script, include the specific change in the email to bring the script online because the singularity image needs to be updated accordingly.
#
#
# python DesSupIndicator.py lc 100 if bookended 100 2 4 --dynsp


import argparse, numpy as np, string


def main(Projtype, Tobsreq, Obsmode, Obsstrategytype, Parallelobs, Nruns, Nsap, Arrconstr, Schedconstr, Resptel, Targlistupd, Npipxobs, Dynsp):
	"""
	...
	if Obsmode.lower() == 'bf+if' or Obsmode.lower() == 'if+bf': degreeofsupport_Obsmode = 'High' # CHANGED
...
if __name__ == "__main__":
	...
	main(Projtype, Tobsreq, Obsmode, Obsstrategytype, Parallelobs, Nruns, Nsap, Arrconstr, Schedconstr, Resptel, Targlistupd, Npipxobs, Dynsp)